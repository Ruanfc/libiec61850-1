...
// };
typedef struct sSocket sEthernetSocket;
// struct sSocket {
//     int fd;
...
typedef struct sHandleSet sEthernetHandleSet;
// struct sHandleSet {
//     LinkedList sockets;
...
EthernetHandleSet_addSocket(EthernetHandleSet self, const EthernetSocket sock)
{
    ...
    // }
    Handleset_addSocket((EthernetHandleSet)self, (EthernetSocket)sock);
}
...
EthernetHandleSet_removeSocket(EthernetHandleSet self, const EthernetSocket sock)
{
    ...
    //     }
    // }
    Handleset_removeSocket((EthernetHandleSet)self, (EthernetSocket)sock);
}
...
EthernetHandleSet_waitReady(EthernetHandleSet self, unsigned int timeoutMs)
{
    ...
    // return result;
    return Handleset_waitReady((EthernetHandleSet)self, timeoutMs);
}
...
EthernetHandleSet_destroy(EthernetHandleSet self)
{
    ...
    // GLOBAL_FREEMEM(self);
    Handleset_destroy((EthernetHandleSet)self); 
}
...
Ethernet_getInterfaceMACAddress(const char* interfaceId, uint8_t* addr)
{
    ...
    // }
    ESP_ERROR_CHECK(esp_read_mac(addr, ESP_MAC_WIFI_STA)); // Não interessa a interfaceId
}
